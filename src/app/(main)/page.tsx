...
export default async function TodayPage() {
  ...
  const todayRoutine = routines.find((r) => r.days.includes(todayWeekday)) ?? routines[0] ?? null;
  ...
  const heroMessage =
    completionMessages.find((m) => today.completionRate >= m.min)?.message ?? "";
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="glass-card flex items-center gap-6 px-6 py-7">
        <ProgressRing percent={today.completionRate} />
        <div>
        ...
        <div className="flex flex-col gap-3">
          {todayExercises.length === 0 && (
            <Link href="/workout" className="surface-card p-4 text-center text-[13px] text-text-muted">
              오늘 요일에 등록된 운동이 없어요. 눌러서 루틴을 만들어보세요.
            </Link>
          )}
      ...
      <section>
        ...
        <div className="surface-card p-4">
          <p className="min-h-[44px] text-[13px] leading-[1.6] text-text-secondary">{today.memo}</p>
        </div>
      </section>
    </div>
  );
